...
import type { Household, Service, Rate, DeliveryRecord, PausePeriod, Payment, DeliveryStatus, ThemeMode } from '../types';
...
interface DailioState {
  ...
  getServiceRecords: (serviceId: string, year: number, month: number) => DeliveryRecord[];
}
...
export const useStore = create<DailioState>((set, get) => ({
  ...
  init: () => {
    ...
    const pauses = storage.getPauses();
    const payments = storage.getPayments();
    const theme = storage.getTheme();
    applyTheme(theme);

    if (typeof window !== 'undefined' && !(window as unknown as { __themeListener?: boolean }).__themeListener) {
      (window as unknown as { __themeListener?: boolean }).__themeListener = true;
  ...
  setRecord: (serviceId, date, status, quantity, note = '') => {
    ...
    if (existing) {
      updated = records.map(r =>
        r.id === existing.id
          ? { ...r, status, quantity, note, updatedAt: new Date().toISOString() }
          : r
      );
    } else {
      ...
      if (shouldWrite) {
        if (existing) {
          updated = updated.map(r =>
            r.id === existing.id
              ? { ...r, status, quantity, source: 'bulk', updatedAt: new Date().toISOString() }
              : r
          );
        } else {
          updated.push({
  ...
  addPayment: (amountMinor, note, paidAt) => {
    const { household, payments, currentYear, currentMonth } = get();
  ...
  getServiceRecords: (serviceId, year, month) => {
    const prefix = `${year}-${String(month + 1).padStart(2, '0')}`;
    return get().records.filter(r => r.serviceId === serviceId && r.date.startsWith(prefix));
  },
}));
